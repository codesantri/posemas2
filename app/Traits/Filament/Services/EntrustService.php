...
trait EntrustService
{
     ...
     */
    public static function handleCreate(array $data): Entrust
    {
        DB::beginTransaction();
        ...
        try {
            ...
            $entrust = new Entrust();
            $entrust->transaction_id = $transaction->id;
            $entrust->customer_id = $data['customer_id'];
            $entrust->total_payment = $data['total_payment'];
            $entrust->status = 'pending';
            $entrust->save();

            // Simpan detail produk
            foreach ($data['items'] ?? [] as $item) {
                EntrustDetail::create([
    ...
    public static function getEditing(Model $record): array
    {
        ...
        foreach ($record->entrustDetails as $item) {
            $itemData = [
                'product_id' => $item->product_id,
                'quantity' => $item->quantity,
                'price' => $item->price,
    ...
    public static function getUpdate(Entrust $entrust, array $data): array
    {
    ...
    public static function getUpdating(Model $record, array $data): Model
    {
        ...
        try {
            ...
            $record->entrustDetails()->delete();

            // Recreate sale details
            foreach ($data['items'] ?? [] as $item) {
                EntrustDetail::create([
                    'entrust_id'    => $record->id,
                    'product_id' => $item['product_id'],
                    'quantity'   => $item['quantity'],
                    'price'      => $item['price'],
                    'subtotal'   => $item['subtotal'],
