...
<body>
    ...
    <div class="container mb-4" style="margin-top: 4cm; margin-left: 8.2cm;">
        <div class="row ">
            <div class="col">
                <span class="ms-5">{{ $invoice->purchase->customer->name }}</span>
            </div>
            <div class="col text-end">
                <span class="ms-5">{{ $invoice->purchase->customer->address }}</span>
            </div>
        </div>
    </div>
    ...
    <table class="table" style="margin-left: 5cm; margin-top:2.3rem">
        <tbody>
            @php
            $maxRows = 5;
            $details = $invoice->purchase->purchaseDetails;
            $count = $details->count();
            @endphp
            ...
            <tr>
                ...
                <td style="width: 45%" class="border-0 py-1"><span class="ms-2">{{ $item->product->name }}</span></td>
                <td class="border-0 py-1 mx-3" style="text-align: start !important;">
                    Rp.{{ number_format($item->product->karat->buy_price, 0, ',', '.') }}
                </td>
            </tr>
            @endforeach
                    ...
                    <td class="py-1 border-0" colspan="3">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="col-6 ms-5">
                                <strong class="mx-4">{{ number_format($invoice->purchase->total_amount, 0, ',', '.')
                                    }}</strong>
                            </div>
                        </div>
                    </td>
                ...
                <tr>
                    <td class="py-2 border-0" colspan="3">
                        <div class="row">
                            <div class="col-2">
                                <p class="mb-1 ps-3">
                                    <strong>CS:</strong> {{ $invoice->purchase->user->name }}
                                </p>
                                <p class="mb-1 ps-3">
                                    {{ \Carbon\Carbon::parse($invoice->transaction_date)->locale('id')->isoFormat('D
                                    MMMM Y') }}
                            ...
                            <div class="col-10">
                                <h5 class="fow-bold mt-1" style="font-style: italic;">
                                    {{ spelledOut($invoice->purchase->total_amount) }}
                                </h5>
                            </div>
                        </div>
                    </td>
